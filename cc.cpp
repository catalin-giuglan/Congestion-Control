...
CCSrc::CCSrc(EventList &eventlist)
    : EventSource(eventlist,"cc"), _flow(NULL) // Remove the semicolon here
{
    _alpha = 1.0;
    _g = 0.0625;   // Set estimation gain
...
void CCSrc::processAck(const CCAck& ack) {    
    ...
    uint64_t bytes_acked = ackno - _highest_sent;
    _bytes_acked += bytes_acked;

    if (ack.is_ecn_marked()){
        _bytes_marked += bytes_acked;
    }
        ...
        // Adjust congestion window based on Alpha
